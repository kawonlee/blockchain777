...
const RegistContainer = () => {
  ...
  const onClick = (userId, userPw, userName) => {
    ...
    // 6. store의 dispatch를 호출했다. 매개변수로 action의 regist를 호출해 return 값을 전달했다. => dispatch 호출보다 action의 regist 호출이 먼저 실행된다.
    // 7. action의 regist를 호출했다. userId, userPw, userName를 매개변수로 전달했다.
    // 10, dispatch를 호출했다. action.regist의 return 값(반환값, == 액션)을 매개변수로 전달했다.
    // 11. dispatch는 reducer를 호출하여 액션을 전달한다.
...
// div가 있다. component/ 아니라면 container

// 화살표 함수
// () => ({}) << 왼쪽 () 안에 있는 것이 function()의 괄호 안에 있는 것과 같다.
// 즉 왼쪽 () 안의 것이 매개 변수이다. =>(화살표) 오른쪽은 함수의 return 값이다. (현재는 {}, 객체를 반환한다.)
// () => [] << 왼쪽 ()는 받는 매개변수이다. 오른쪽은 함수가 return하는 []배열이다.
// () => {} << 왼쪽 ()는 받는 매개변수이다. 오른쪽 {}(중괄호)는 함수의 내용이다.
