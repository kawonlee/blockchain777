...
// - useSelector를 사용하면 state변화 시 랜더링을 다시 해준다.(리랜더링)
// 순서 총괄 : action -> dispatch -> reducer -> state

// thunk 사용 시 실행 순서
// 1. dispatch(action)을 호출한다.
// 2. action이 함수인지 확인하여 함수라면 reducer에 전달하는 것이 아닌 action 함수 자체를 호출한다. << 확인은 thunk가 해준다. 전달하는 action함수는 async, await가 가능하다.
// 3. action이 객체인 경우 기존의 redux 실행순서와 마찬가지로 작동한다.
...
const asyncIncrement = () => {
  ...
  return async (dispatch, getState) => {
    ...
    try {
      // await를 사용한 함수의 호출이 에러가 발생할수도 있기 때문에 try, catch(reject) 구문을 사용한다.
      // 4. promiseTime함수를 호출하고 끝나기를 기다려서 result에 결과(resolve)를 정의한다.
...
const asyncDecrement = () => {
  return async (dispatch, getState) => {
    ...
    try {
      ...
      dispatch({ type: result });
      // return await promiseTime(TYPE.DECREMENT, 1000); << 바로 return 시 state에 적용되지 않는다.
    } catch (error) {
      dispatch({ type: "error", payload: error });
    }
  };
...
export const action = { increment, decrement, asyncDecrement, asyncIncrement };
