...
// 사용할때는 new Set() 를 변수에 정의하여 변수를 사용한다.
// clear(), add(), size
...
function reset() {
  ...
  while (comSet.size < 3) {
    ...
    // 만약 중복되는 수라면 알아서 중복되지 않게 처리해준다. (5번째 줄에서 new Set()으로 정의해줬기때문이다.)
  }
  //   console.log(comSet);
  comSel = [...comSet];
  // ...은 (스프레드 =)전개 구문이라고 한다.
...
function selectNum() {
  ...
  while (playerSel.length !== 3) {
    ...
    if (!playerSel) return alert("포기한거냐 애송이");
  ...
  comSel.forEach((item, index) => {
    // forEach의 매개변수함수(배열/문자열의 아이템, 인텍스(순서)) => {내용}
