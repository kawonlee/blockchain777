...
//  - binary Data : 이진데이터, 컴퓨터가 저장, 처리 등등 계산을 할 때 사용한다.
//  - RAM 등에 저장되는 데이터를 저장된 그대로 보여준다. (단, 보여줄 때는 16진수로 바꿔서 보여준다.)
...
// URL Encode << url에서 사용하는 포맷 형식

const tempBufferArr1 = Buffer.from([1, 2, 3, 300]);
console.log(tempBufferArr1);
// 300 은 44가 찍힌다. => 왜냐 FF 즉 255까지만 저장할 수 있기 때문에 넘어갈 경우 256을 빼서 남은 값을 저장한다. << 256으로 나눈 나머지만 저장한다.
...
// 16진수는 0x??로 작성할 수 있다. => ex) 0x11 => 17 (16+1)
// 8진수는 앞에 0을 하나 더 작성한다. => ex) 015 = 13(8+5)
...
const server = net.createServer((client) => {
  ...
  client.on("data", (data) => {
    ...
    // Sec-Fetch-Site: none << CORS 설정 : 해당 정보를 요청한 주소가 서버의 주소가 맞는지. 서버의 데이터를 조금이라도 안전하게 지키기 위해 확인한다.
...
Hi Block 7`);
    ...
    // Content-Type : 어떤 데이터로 응답할건지, 어떤 정보를 보낼지에 대한 포맷(확장자)설정, charset < 언어 포맷 설정
