...
// - "test" : "jest"
//      - jest 뒤에 파일명을 입력하면 해당 파일만 확인한다. 5번줄처럼 아무것도 적지 않으면 모든 파일을 읽어온다.
...
describe("이건 테스트의 그룹이다.", () => {
  ...
  // test || it
  // it(name, func)
  // it 또는 test는 name 이라는 이름의 테스트를 실행한다. it이랑 test 섞어서 사용해도 문제 없다.
  ...
  //  - toXXXXXXX 를 매쳐(matcher)라고 부른다. << 엄청 많다. ex) toBe, toEqual 등등
  //  - toBe의 경우 완벽한 일치를 확인하기 때문에 객체 등을 비교할 수는 없다.

  it("이건 안된다 => 이건 된다", () => {
    //   expect({ a: 1 }).toBe({ a: 1 }); << 안되는 애
    // 객체는 서로 다른 것으로 취급한다. 같은 객체가 되려면 변수에 정의해야한다.
    const obj = { a: 1 };
    expect(obj).toBe(obj); // << 되는 애
  });
...
describe("merkle 비교", () => {
  ...
  it("두개의 데이터 암호화 확인", () => {
    // expect(merkleRoot).toBe(secondTreeRoot);
  });

  it("네개의 데이터 암호화 확인", () => {
    expect(merkleRoot).toBe(thirdTreeRoot);
  });

  it("세개의 데이터 암호화 확인", () => {
    expect(oddMerkleRoot).toBe(oddThirdRoot);
  });
});
