...
- Metadata란
  ...
  - 데이터에 관한 구조화된 데이터 / 다른 데이터를 설명해주는 데이터
    - 사전에서 ㄱ, ㄴ, ㄷ,순으로 나타난다. [ㄱ, ㄴ, ㄷ] 자체를 메타데이터
      - ["가을 : 계절이다.", "사과 : 과일이다.", "책상 : 물건이다."]
      - 메타데이터 : [가나다순, 과일, 계절]
      - 사진이 있으면 그 사진이 언제 찍혔고 등의 정보를 메타데이터라고 함
...
```
    ...
    - URI : Uniform Resource Identifier의 약어로 특정 데이터를 식별하는 식별자, 리소스를 구분하는 고유 문자열
...
contract ERC721 is IERC721, IERC721Metadata {
  ...
  // IERC721Metadata에서 선언되었기 때문에 override 필요 (metadata쪽에서 virtual을 가짐(선언하지않더라도))
  ...
  function approve(address _to, uint _tokenId) external override {
    address owner = _owners[_tokenId];
    // address owner = ownerOf(_tokenId) << 수수료 들기 때문에 이렇게 사용안함
    // 위임할 토큰의 주인 확인
  ...
  // 트랜잭션 보낸 계정의 모든 토큰에 대한 권한을 _operator에게 _approved로 설정한다. | _approved == true면 모든 권한 위임 | _approved == false면 권한 위임 취소
...
contract ERC721Enumerable is ERC721 {
  ...
  function _beforeTokenTransfer(
    ...
    if (_from == address(0)) {
      ...
      if (tokenIndex != latestTokenIndex) {
        ...
        // 소유자의 (보내는위치)토큰 index의 ID를 소유자의 마지막 토큰의 ID로 정의
  ...
  // 소유자와 소유자 기준의 토큰 index로 토큰 Id 검색
}
```
