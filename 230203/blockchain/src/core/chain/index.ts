...
class Chain implements IChain {
  ...
  constructor() {
    ...
    const transaction = new Transaction(
      [new TxIn(`가원이의 제네시스 블록 ${new Date()}`, 0)],
      []
    );
  ...
  replaceChain(_chain: Array<IBlock>): TResult<undefined, string> {
    ...
    this.chain.forEach((_block: IBlock) => {
      // 트랜잭션 풀을 업데이트하고(삭제할건 삭제하고 추가할건 추가하기 위함)
  ...
  updateUTXO(_tx: Transaction) {
    ...
    let temp = utxos.filter((item) => {
      const txIn = _tx.txIns.find(
        (item1) =>
          item1.txOutId === item.txOutId && item1.txOutIndex === item.txOutIndex
        // 트랜잭션의 txIns에 들어갔다는 얘기는 input으로 넣어서 사용했다는 얘기가 된다.
        // 그럼 기존의 utxos에서 사용한 utxo들을 빼야한다.
        // 그래서 txIns와 utxos를 비교, 검색해서 나오면 filter에서 걸러진다.
      );
      return !txIn;
      // return값은 find로 찾아진 애들을 제외한 나머지를 반환한다.
    });
  ...
  updateTxPool(_newBlock: IBlock): void {
    ...
    for (let i = 0; i < tempTx.length; ++i) {
      ...
      for (let j = 0; j < txPool.length; ++j) {
        ...
        // 기존 트랜잭션 풀과 새로 사용된 트랜잭션들(블록 내의 트랜잭션)을 비교해서 사용되지 않은 트랜잭션을 새로운 배열에 가져온다.
      }
      txPool = tempTxPool;
