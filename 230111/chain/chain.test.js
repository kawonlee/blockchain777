...
// npx jest --verbose << 두개 이상 돌리면 안뜨던 어떤 라인이 test했는지를 다시 띄워줌
...
describe("chain test", () => {
  ...
  describe("addBlock test", () => {
    ...
    it("잘못된 데이터로 블록 추가 확인", () => {
      // it이 시작하기 전에 위에서 설정된 beforeEach가 실행된다.
      console.log(chain.chain); // 여기서의 체인은 새로 생성된 체인
      chain.addBlock("비정상데이터");
      console.log(chain.chain); // 비정상 데이터로 인해 chain에 추가되지 않는다.
  ...
  describe("add2Chain", () => {
    it("블록 생성 후 추가 확인", () => {
      const newBlock = new Block(["qqqq"], chain.lastBlock);
      chain.add2Chain(newBlock);
      expect(chain.chain).toHaveLength(2);
    });
    it("잘못된 블록 생성 후 추가 확인", () => {
      const newBlock = new Block("qqqq");
      chain.add2Chain(newBlock);
      expect(chain.chain).toHaveLength(1);
    });
  });
  ...
  describe("lastBlock check", () => {
    it("마지막 블록 확인", () => {
      chain.addBlock(["lkw"]);
      const newBlock = new Block(["qqq"], chain.lastBlock);
      chain.add2Chain(newBlock);
