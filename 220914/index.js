...
// childNodes는 자식 노드들을 가져온다.
// 리액트는 띄어쓰기를 없앤다. [>> 용량이적다. / Node에선 띄어쓰기를 text로 인식한다.]
// tag << element Node는 객체로 감싸져있지 않는 객체를 말한다.
...
[...children].forEach(function (elem, index) {
  ...
  // forEach는 배열의 아이템을 하나하나 가져와서 매개변수함수로 전달된 함수에 매개변수로
  //전달해서 함수를 호출한다.
  elem.onmouseover = () => {
  ...
  elem.onclick = function () {
    ...
    /*{if (elem.classList.contains("on")) {
      // contains 메서드는 매개변수로 전달된 문자열(string)이 클래스에 포함되어 있는지
      //를 확인한다.
    ...
    // toggle 메서드 = 클래스가 있으면 없애고, 없으면 추가한다.
  };
});
