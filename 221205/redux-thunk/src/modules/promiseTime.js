...
const promiseTime = (type, time) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      // 매개 변수로 전달된 시간 후에 매개변수로 전달된 타입을 반환한다.
...
//     try {
//       // try 스코프 내의 코드를 실행한다.
//       // 도중에 오류가 발생하면 아래 catch의 error로 전달한다.
//       setTimeout(() => {
//         resolve(text);
//       }, time);
//     } catch (error) {
...
//     // 콜백함수의 매개변수는 resolve에 전달된 매개변수(text를 result로)를 받는다.
//     console.log(result);
//     return callbackPromise("프라미스2", 1000);
//     // 다음 프라미스(작업)는 return으로 반환하며 아래에 then으로 위의 프라미스(현재는 82번줄)의 결과를 받을 수 있다.
//   })
//   .then((result) => {
//     console.log(result);
